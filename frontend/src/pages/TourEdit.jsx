...
const TourEdit = () => {
  ...
  const handleSuggestionClick = async (suggestion) => {
    ...
    try {
      const wikiRes = await axios.get(`https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(primaryName)}&prop=pageimages&format=json&pithumbsize=1000&origin=*`);
      const pages = wikiRes.data.query.pages;
      const pageId = Object.keys(pages)[0];
      if (pageId !== "-1" && pages[pageId].thumbnail) {
        setImage(pages[pageId].thumbnail.source);
      } else {
